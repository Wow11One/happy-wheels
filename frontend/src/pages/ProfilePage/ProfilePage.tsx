...
const ProfilePage = () => {
    ...
    const getAuthClient = async () => {
        ...
        console.log('await canisterActor.get_user_by_principal(identity.getPrincipal().toString()', currUser)
                    ...
                    {activeTab === "profile" && (
                        <div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-gray-400 text-sm font-medium mb-1">Account Type</label>
                                    <p className="text-white font-mono">{currentUser?.is_service ? "Service Provider" : "Customer"}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        {!isCurrentUserProfile() && !!currentUser?.is_service && <button>Give tires</button>}
                                        {isCurrentUserProfile() && <button onClick={() => navigate(ApplicationRoutes.TyreCreateForm)}>Add tire</button>}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {tires && tires.length > 0 ? (
                                ...
                                <div className="text-center p-4">
                                    <p>{isCurrentUserProfile() ? 'You don\'t' : 'User doesn\'t'} have any tires listed yet.</p>
                    ...
                    {activeTab === "transactions" && (
                        <div>
                            ...
                            {transactions && transactions.length > 0 ? (
                                        ...
                                        <tbody>
                                            {user.transactions.map((transaction, index) => (
                                                <tr key={index} className="border-b border-gray-800">
                                                    ...
                                                    <td className={`py-3 px-4 ${transaction?.amount >= 0 ? 'text-green-400' : 'text-red-400'}`}>${transaction.amount}</td>
                                ...
                                <div className="text-center p-4">
                                    <p>{isCurrentUserProfile() ? 'You don\'t' : 'User doesn\'t'} have any transactions yet.</p>
                                </div>
                            )}
                        </div>
                    )}
